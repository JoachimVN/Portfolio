...
function requireBigInteger() {
  ...
  (function(module) {
    var bigInt2 = (function(undefined$1) {
      ...
      function add2(a, b) {
        var l_a = a.length, l_b = b.length, r = new Array(l_a), carry = 0, base = BASE2, sum, i3;
        for (i3 = 0; i3 < l_b; i3++) {
          sum = a[i3] + b[i3] + carry;
          carry = sum >= base ? 1 : 0;
          r[i3] = sum - carry * base;
      ...
      function addSmall(a, carry) {
        var l = a.length, r = new Array(l), base = BASE2, sum, i3;
        for (i3 = 0; i3 < l; i3++) {
          sum = a[i3] - base + carry;
      ...
      function subtract2(a, b) {
        var a_l = a.length, b_l = b.length, r = new Array(a_l), borrow = 0, base = BASE2, i3, difference;
        for (i3 = 0; i3 < b_l; i3++) {
      ...
      function subtractSmall(a, b, sign) {
        var l = a.length, r = new Array(l), carry = -b, base = BASE2, i3, difference;
        for (i3 = 0; i3 < l; i3++) {
      ...
      function multiplyLong(a, b) {
        var a_l = a.length, b_l = b.length, l = a_l + b_l, r = createArray(l), base = BASE2, product, carry, i3, a_i, b_j;
        for (i3 = 0; i3 < a_l; ++i3) {
          a_i = a[i3];
          for (var j = 0; j < b_l; ++j) {
            b_j = b[j];
      ...
      function multiplySmall(a, b) {
        var l = a.length, r = new Array(l), base = BASE2, carry = 0, product, i3;
        for (i3 = 0; i3 < l; i3++) {
      ...
      function square(a) {
        var l = a.length, r = createArray(l + l), base = BASE2, product, carry, i3, a_i, a_j;
        for (i3 = 0; i3 < l; i3++) {
          a_i = a[i3];
          carry = 0 - a_i * a_i;
          for (var j = i3; j < l; j++) {
      ...
      function divMod1(a, b) {
        var a_l = a.length, b_l = b.length, base = BASE2, result = createArray(b.length), divisorMostSignificantDigit = b[b_l - 1], lambda = Math.ceil(base / (2 * divisorMostSignificantDigit)), remainder = multiplySmall(a, lambda), divisor = multiplySmall(b, lambda), quotientDigit, shift, carry, borrow, i3, l, q;
      ...
      function divModSmall(value, lambda) {
        var length = value.length, quotient = createArray(length), base = BASE2, i3, q, remainder, divisor;
        remainder = 0;
      ...
      function millerRabinTest(n, a) {
        var nPrev = n.prev(), b = nPrev, r = 0, d, i3, x;
        while (b.isEven()) b = b.divide(2), r++;
        next: for (i3 = 0; i3 < a.length; i3++) {
          if (n.lesser(a[i3])) continue;
          x = bigInt2(a[i3]).modPow(b, n);
      ...
      function randBetween(a, b, rng2) {
        ...
        for (var i3 = 0; i3 < digits.length; i3++) {
          var top = restricted ? digits[i3] + (i3 + 1 < digits.length ? digits[i3 + 1] / BASE2 : 0) : BASE2;
...
function splitBigInt(value) {
  ...
  for (let i2 = 0; i2 < PARTS; i2++) {
    parts[PARTS - 1 - i2] = fromHexReverseArray(sum, i2 * PARTS, PARTS);
  }
  return parts;
}
var HighLow = class _HighLow {
  ...
  static fromBit(index) {
    ...
    for (let i2 = 0; i2 < PARTS; i2++) {
      parts[PARTS - 1 - i2] = i2 === offset ? 1 << index - offset * SMALL_INT : 0;
    }
  ...
  and({ parts }) {
    return new _HighLow(this.parts.map((v, i2) => v & parts[i2]));
  }
  or({ parts }) {
    return new _HighLow(this.parts.map((v, i2) => v | parts[i2]));
  }
  ...
  equals({ parts }) {
    return this.parts.every((v, i2) => v === parts[i2]);
  }
  /**
  ...
  toString() {
    ...
    this.parts.forEach((value, offset) => {
      ...
      for (let i2 = 0; i2 < 4; i2++) {
        array[i2 + offset * 4] = hex[4 - 1 - i2] || 0;
      }
    });
...
function flagOrMultiple(...flags) {
  let result = flags[0];
  for (let i2 = 1; i2 < flags.length; i2++) {
    result = flagOr(result, flags[i2]);
  }
  return result;
}
...
function add(x, y) {
  var carry, d, e, i2, k, len, xd, yd, Ctor = x.constructor, pr = Ctor.precision;
  if (!x.s || !y.s) {
    if (!y.s) y = new Ctor(x);
...
var divide = /* @__PURE__ */ (function() {
  ...
  return function(x, y, pr, dp) {
    var cmp, e, i2, k, prod, prodL, q, qd, rem, remL, rem0, sd, t, xi, xL, yd0, yL, yz, Ctor = x.constructor, sign = x.s == y.s ? 1 : -1, xd = x.d, yd = y.d;
...
function exp(x, sd) {
  var denominator, guard, pow, sum, t, wpr, i2 = 0, k = 0, Ctor = x.constructor, pr = Ctor.precision;
...
function round(x, sd, rm) {
  var i2, j, k, n, rd, doRound, w, xdi, xd = x.d;
  ...
  if (rm !== void 0) {
    ...
    (i2 > 0 ? j > 0 ? w / mathpow(10, n - j) : 0 : xd[xdi - 1]) % 10 & 1 || rm == (x.s < 0 ? 8 : 7));
  }
  if (sd < 1 || !xd[0]) {
    if (doRound) {
...
function subtract(x, y) {
  var d, e, i2, j, k, len, xd, xe, xLTy, yd, Ctor = x.constructor, pr = Ctor.precision;
  if (!x.s || !y.s) {
    if (y.s) y.s = -y.s;
    else y = new Ctor(x);
    return external ? round(y, pr) : y;
